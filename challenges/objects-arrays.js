...
console.log(`${dinosaur_1.name}' weighed about ${dinosaur_1.weight}kg.`);
...
console.log(`A ${dinosaur_1.name} goes ${dinosaur.roar}.`);
...
for (index; index < graduates.length; index++) { 
  let graduatesDescription = graduates[index];

  // Request 1:
  universities.push(graduatesDescription.university);
  var universities_A_Z = universities.sort(); 

  // Request 2:
  contactInfo.push(graduatesDescription.first_name + ` ` + graduatesDescription.email);

  // Request 3:
  if (graduatesDescription.university.indexOf("Uni") != -1) {
    unisWithUni.push(graduatesDescription.university);
  }
}
...
console.table(universities_A_Z);
